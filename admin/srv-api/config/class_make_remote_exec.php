<?php
namespace WPRSSFeddImporter\SrvApi\Endpoint;
...
class Make_Remote_Exec
{
    ...
    private function check_is_plugin_aktiv(){
        ...
        if(!get_option("{$this->basename}_update_config")->aktiv){
            add_action('admin_notices',array($this, 'remove_wp_rss_importer_admin_page' ));
        }
    }

    public function remove_wp_rss_importer_admin_page()
    {
        echo sprintf('<div class="notice notice-error is-dismissible"><p><b>%s</b> - %s %s</p></div>',__('RSS Feed Importer', 'wp-rss-feed-importer') ,__('Plugin has been disabled.','wp-rss-feed-importer'),__('Contact the support.' ,'wp-rss-feed-importer'));
    }
